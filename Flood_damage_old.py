...
from Water_spread import water_height_figures,w,land_height,landscape_shape,steps_to_reduce,figures_ratio,water_height_river
...
9. Nature
"""
# This function turns the landuse_num to strings used by other functions
def landuse_num_to_category(landuse_num):
    #print (landuse_num)
    num2landuse = {
        1: 'infrastructure',
        2: 'residential',
    ...
    category = num2landuse.get(landuse_num)
    assert category is not None, "Invalid landuse_num"
    return category
...
"""
@nb.njit(fastmath=True)
def max_damage_calc(category,i,j):
...
# =============================================================================
@nb.njit(fastmath=True)
def tile_financial_dmg(water_level,land_class,i,j):
    damage = 0
    damage_factor = 0
    max_damage = max_damage_calc(land_class,i,j)
...
def total_damage_calc(water_height_figures,landuse,run4all):
    ...
    for step in range(len(water_height_figures)):
        ...
        for i in nb.prange(landscape_shape[0]):
            ...
            for j in nb.prange(landscape_shape[1]):
                ...
                tile_damage = tile_financial_dmg(max_water_height,landuse_category,i,j)
                total_damage += tile_damage
        # Inflation calculation:
        total_damage = total_damage*1.8
        minutes,hours,days = min2dayhours(step)
...
lotta_damage = total_damage_calc(water_height_figures,landuse,False)
end_time = time.time()
